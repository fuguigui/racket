...
(define (install-raise-package)
  ...
  (define (raise-rational x)
    (make-real (/ ((get 'rational 'numer) x)
                  ((get 'rational 'denom) x))))
  (define (raise-real x)
...
(define (apply-generic op . args)
  (define (order tag lst);保证一定能在lst中找到tag
    (if(eq? tag (car lst))
       1
       (+ 1 (order tag (cdr lst)))))
  
  (define (higher? tag1 tag2)
    (define standard (list 'integer 'rational 'real 'complex))
    (let ((o1 (order tag1 standard))
          (o2 (order tag2 standard)))
      (if(> o1 o2)
         #t
         #f)))
      ...
      (if proc
          ...
          (if (= (length args) 2)
              (let ((tag1 (car type-tags))
                    ...
                    (a1 (car args))
                    (a2 (cadr args)))
                (let ((t1>t2 (higher? tag1 tag2)))
                  (if t1>t2
                      (apply-generic op a1 ((get 'raise tag2) a2))
                      (apply-generic op ((get 'raise tag1) a1) a2))))
...
(define (display-obj obj)
  (define tag (car obj))
  (cond ((eq? tag 'integer)
         (displayln (list tag (cdr obj))))
        ((eq? tag 'rational)
         (displayln (list tag (cadr obj) (cddr obj))))
        ((eq? tag 'real)
         (displayln (list tag (cdr obj))))
        ((eq? tag 'complex)
         (displayln (list tag (caddr obj) (cdddr obj))))
        (else
         (error "No correponding display function"))))
