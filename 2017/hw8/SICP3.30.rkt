...
(define (ripple-carry-adder wire-lstA wire-lstB wire-lstS wireCN)
  (define (full-adder-loop wa wb ws c)
          (if (null? (cdr wa))
              (full-adder(car wa)(car wb) c (car ws) wireCN)
              (let((temp (make-wire)))
              (begin
                (full-adder (car wa)(car wb) c (car ws)temp)
                (full-adder-loop (cdr wa)(cdr wb) (cdr ws) temp)))))
  (full-adder-loop wire-lstA wire-lstB wire-lstS (make-wire)))
